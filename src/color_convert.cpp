...
static inline int yuv_to_bgr(int y, int u, int v) {
    int c = y - 16;
    int d = u - 128;
    int e = v - 128;
...
void rgbx_to_bgr24(const uint8_t* src, int src_pitch,
    ...
    for (int row = 0; row < height; ++row) {
        ...
        for (int col = 0; col < width; col++) {
            out[col * 3 + 0] = line[col * 4 + 2];  // R -> B
            out[col * 3 + 1] = line[col * 4 + 1];  // G -> G
            out[col * 3 + 2] = line[col * 4 + 0];  // B -> R
        }
    }
}
...
void p216_to_bgr24(const uint8_t* src_y, int y_pitch,
                  ...
                  uint8_t* dst, int width, int height) {
    for (int row = 0; row < height; ++row) {
        const uint16_t* y_line = reinterpret_cast<const uint16_t*>(src_y + row * y_pitch);
        const uint16_t* uv_line = reinterpret_cast<const uint16_t*>(src_uv + (row / 2) * uv_pitch);
        uint8_t* out = dst + row * width * 3;

        for (int col = 0; col < width; col += 2) {
            int u  = uv_line[col * 2 + 0] >> 8;
            int y0 = y_line[col] >> 8;
            int v  = uv_line[col * 2 + 1] >> 8;
            int y1 = y_line[col + 1] >> 8;
